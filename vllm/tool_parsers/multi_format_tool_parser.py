...
class MultiFormatToolParser(ToolParser):
    ...
    def __init__(
        ...
        self._default_chat_template_kwargs = chat_template_kwargs or {}
        self._tool_format_from_kwargs(self._default_chat_template_kwargs)
        self._delegate: ToolParser | None = None

    @classmethod
    def _tool_format_from_kwargs(cls, chat_template_kwargs: dict[str, Any]) -> str:
        ...
        return cls._validate_tool_format(raw_tool_format)

    def _tool_format_for_request(self, request: ToolParserRequest) -> str:
        chat_template_kwargs = {
            **self._default_chat_template_kwargs,
            **(request.chat_template_kwargs or {}),
        }
        return self._tool_format_from_kwargs(chat_template_kwargs)

    def _delegate_for_format(self, tool_format: str) -> ToolParser | None:
        ...
        if self._delegate is None:
            ...
            self._delegate = Qwen3EngineToolParser(self.model_tokenizer, self.tools)
    ...
    def adjust_request(self, request: ToolParserRequest) -> ToolParserRequest:
        tool_format = self._tool_format_for_request(request)
        if delegate := self._delegate_for_format(tool_format):
            return delegate.adjust_request(request)
    ...
    def extract_tool_calls(
        ...
        tool_format = self._tool_format_for_request(request)
        if delegate := self._delegate_for_format(tool_format):
            return delegate.extract_tool_calls(model_output, request)
    ...
    def extract_tool_calls_streaming(
        ...
        tool_format = self._tool_format_for_request(request)
        if delegate := self._delegate_for_format(tool_format):
            return delegate.extract_tool_calls_streaming(
                previous_text,
                current_text,
                delta_text,
                previous_token_ids,
    ...
    def _schema_arg_type(
        ...
        for tool in tools:
            ...
            if candidate_name != tool_name or parameters is None:
                continue
