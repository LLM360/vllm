...
class XllmRMSNorm(RMSNorm):
    ...
    def forward(self, x, residual=None):
        ...
        # Reuse vLLM's RMSNorm CUDA kernel on each group, then apply XLLM's
        # per-channel affine weight after restoring the original shape.
        ops.rms_norm(
            y_grouped,
            x_grouped,
...
class XllmAttention(nn.Module):
    ...
    def forward(
        ...
        if self.rope_head_dim == self.head_dim:
            ...
            k_ = k_.reshape(
                *k_.shape[:-1],
                self.total_num_kv_heads * num_kv_head_replicas,
                self.head_dim,
            ...
            q_ = permute_to_hf(
                torch.cat([permute_to_xllm(q_rope), q_nope], dim=-1)
            ).reshape(*q_.shape[:-2], -1)
            k_ = permute_to_hf(
                torch.cat([permute_to_xllm(k_rope), k_nope], dim=-1)
            ).reshape(*k_.shape[:-2], -1)

            q = q_.split(q_.shape[-1] // tp_size, dim=-1)[tp_rank]
            k = k_.split(k_.shape[-1] // (tp_size // num_kv_head_replicas), dim=-1)[
...
class XllmModel(nn.Module, EagleModelMixin):
    def __init__(
        self,
        *,
        vllm_config: VllmConfig,
        prefix: str = "",
        decoder_layer_type: type[torch.nn.Module] = XllmDecoderLayer,
    ):
        super().__init__()
