...
def test_request_tool_call_format_overrides_parser_default():
    parser = make_parser("xml")
    request = make_request()
    request.chat_template_kwargs = {"tool_call_format": "python"}

    extracted = run_tool_extraction_nonstreaming(
        parser,
        '<tool_call>get_weather(city="Tokyo")</tool_call>',
        request,
    )
